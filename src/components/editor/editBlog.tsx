...
const EditBlog = () => {
  ...
  // Function to update content consistently
  const updateContent = () => {
    if (quillRef.current) {
  ...
  useEffect(() => {
    ...
    // Fetch blog data after editor is initialized
    fetchBlog();
  ...
  // Image resize and alt text functionality
  useEffect(() => {
    if (!quillRef.current) return;
  ...
  const uploadthumbnail = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setFormData((prevFormData) => ({ ...prevFormData, thumbnail: data.secure_url }));
        ...
        <div style={{ width: "90%", margin: "auto" }}>
          <form>
            <div className="mt-4 mb-4">
              ...
              {formData?.thumbnail ? (
                <a href={formData?.thumbnail} target="_blank" rel="noopener noreferrer">
